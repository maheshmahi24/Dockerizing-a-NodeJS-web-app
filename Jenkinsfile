    ...
    stage('Deploy docker'){
      steps {
		sh '''
                          ...
                          ssh -i /var/lib/jenkins/.ssh/application.pem -o StrictHostKeyChecking=no ubuntu@ec2-18-141-143-91.ap-southeast-1.compute.amazonaws.com  'bash -s' ./deploy.sh \${BUILD_NUMBER}
